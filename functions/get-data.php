...
function settings_data(){
    ...
    $firstEntryStmt = $db->prepare("SELECT MIN(created_at) AS first_entry_time FROM `water_data` WHERE level < 0 AND DATE(`created_at`) = CURDATE()");
    ...
    $alertResultStmt = $db->prepare("SELECT COUNT(id) AS alerts FROM `water_data` WHERE level < 0 AND DATE(`created_at`) = CURDATE() AND created_at >= :three_minutes_ago");
